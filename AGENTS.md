...
2. `rg -n 'p\.Kind|model\.Kind' internal -g '*.go'` 列出全部分派点
...
触碰配置保存、状态接口或只读部署文档时，必须同时检查 `/api/status`、`PUT /api/config` 和前端只读模式三处契约是否一致：

- HTTP(S) 远程配置源：`config_source.type="remote"`，`config_source.writable=false`，`capabilities.config_write=false`；直接保存返回 `409 config_source_readonly`
- 本地可写配置文件：`config_source.type="local"`，`config_source.writable=true`，`capabilities.config_write=true`；允许 `PUT /api/config`
- 本地配置文件或所在目录不可写：status 仍可读取配置并计算 revision，但返回 `config_source.writable=false` 与 `capabilities.config_write=false`；绕过前端直接保存返回 `409 config_file_not_writable`
- 前端只读模式同时依赖 `capabilities.config_write` 与 `config_source.writable`；文档中不要把本地只读挂载写成 `config_source_readonly`

### CI 流水线检查

Release workflow 按以下顺序执行，任一步失败则阻断后续 job（binaries / docker）：

1. **`go mod verify`**：校验模块缓存完整性
2. **`golangci-lint`**（含 `unused` linter）：检测**包括 `_test.go` 在内**的未使用函数/变量——Go 编译器不报错，但 CI 会拒绝
3. **`gofmt -l .`**：严格格式检查，文件末尾多余空行即失败
4. **`go test ./...`**：标准测试
5. **`go test -tags webui ./...`**：含 Web UI 嵌入资产的构建标签测试
6. **`go vet ./...`**
7. **嵌入资产验证**：`test -s internal/webui/dist/index.html`——确认 Web 前端产物已正确嵌入
8. **`docker build -f Dockerfile .`**：确认生产 Dockerfile 可使用已同步 Web embed 产物构建单镜像

**本地预检命令**：

- 后端/发布基线：`go mod verify && test -z "$(gofmt -l .)" && go test ./... && go test -tags webui ./... && go vet ./... && test -s internal/webui/dist/index.html`
- 涉及 Web 源码：额外跑 `pnpm web:test && pnpm web:typecheck && pnpm web:build`；需要更新嵌入产物时跑 `pnpm web:embed`
- 发布镜像改动：Docker daemon 可用时跑 `docker build -f Dockerfile .`
...
- `rg -n "原始节点|拉取类节点|@all" docs AGENTS.md configs/base_config.yaml` 列出全部引用点
- **概要层文档**（`architecture.md`、决策表）最易遗漏——它们在首次撰写时正确，但后续扩展时通常只改细节层
- 检查范围包括：`docs/`、`AGENTS.md`、`configs/base_config.yaml` 注释
