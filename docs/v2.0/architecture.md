...
本系统是一个使用 Go 实现的单用户 HTTP 服务。它读取一份用户 YAML 配置，拉取 SS/AnyTLS 订阅源、Snell 来源和 VLESS 来源，结合自定义代理、节点分组、服务路由和远程规则集，生成 Clash Meta 或 Surge 配置文件。
...
订阅节点（SS/AnyTLS）+ Snell 节点 + VLESS 节点 + 自定义节点
    │
    ▼
统一中间表示
    │
