...
| 原始节点名 | `HK-01` | `@all` 展开后的具体节点 |
| 保留策略 | `DIRECT`、`REJECT` | 内置 |

说明：

- `@all` 在 Route 阶段展开为具体节点名后写入 Members，中间表示中不出现 `@all` 字面值
...
| 冲突场景 | 策略 | 原因 |
|---------|------|------|
| 跨订阅重名 | 两轮去重：第一轮追加递增后缀（如 `HK-01`、`HK-01②`、`HK-01③`），第二轮检测并解决生成名与原始名的碰撞 | 用户无法控制订阅商命名，需覆盖后缀与原始名碰撞的极端场景 |
| 自定义代理与订阅节点重名 | 报错 | 用户可修改自定义代理名称 |
| 自定义代理之间重名 | 报错 | 用户可修改 |
