...
| 原始节点名 | `HK-01` / `HK-Snell` | `@all` 展开后的具体原始节点（订阅 / Snell / 自定义） |
...
- 链式节点只能引用拉取类节点（订阅 / Snell）作为上游
- `@all` 只包含原始节点（订阅 / Snell / 自定义）
- 节点组和服务组的名称都可被服务组引用
- fallback 必须引用已存在服务组
...
| 冲突场景 | 策略 | 原因 |
|---------|------|------|
| 拉取类节点跨源重名（订阅、Snell 来源共享同一去重池） | 两轮去重：第一轮按出现顺序追加递增后缀（如 `HK-01`、`HK-01②`、`HK-01③`），第二轮检测并解决生成名与原始名的碰撞 | 用户无法控制订阅商/Snell 清单命名，需覆盖后缀与原始名碰撞的极端场景 |
| 自定义代理与拉取类节点（订阅 / Snell）重名 | 报错，错误消息指明冲突源 kind | 用户可修改自定义代理名称 |
| 自定义代理之间重名 | 报错 | 用户可修改 |
