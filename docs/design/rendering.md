...
`GET /api/generate/preview?format=clash|surge` 基于当前运行时配置返回与 `/generate` 完全相同的渲染结果。

`POST /api/generate/preview?format=clash|surge` 接收 `{config}` 草稿配置，先执行临时 `Prepare`，再复用同一套渲染逻辑返回草稿生成结果；它不写文件、不替换 `RuntimeConfig`。
