...
- API 返回 `401 token_required` 或 `401 token_invalid` 时，SPA 展示 token 输入对话框
- API 返回 `401 admin_auth_required` 时，SPA 展示部署配置错误，提示服务端配置 `-access-token` / `SUBCONVERTER_TOKEN`，或在本机/受信网络中显式开启 `-allow-unauthenticated-admin`
