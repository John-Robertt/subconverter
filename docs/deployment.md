...
如需为当前可用的 `/generate` 启用访问控制，可额外设置 `SUBCONVERTER_TOKEN`。客户端下载配置文件时仍需在 URL 上附带 `token` 查询参数。v2.0 Admin API 规划中会复用同一 token，并要求 Web 后台通过 `Authorization: Bearer ...` header 访问 `/api/*`。Admin API 默认要求配置 token；若确需在本机或受信网络中无鉴权使用，必须显式设置 `SUBCONVERTER_ALLOW_UNAUTHENTICATED_ADMIN=true`。
...
`SUBCONVERTER_TOKEN` 只配置在 `api` 服务上。v2.0 Web 页面访问 `/api/*` 时使用 `Authorization: Bearer ...` header；复制 Clash / Surge 订阅链接时，前端再按需把 token 放入 `/generate?token=...` query。若未配置 `SUBCONVERTER_TOKEN`，Admin API 默认返回 `401 admin_auth_required`；只有显式设置 `SUBCONVERTER_ALLOW_UNAUTHENTICATED_ADMIN=true` 时才允许 `/api/*` 无鉴权访问。
...
若不显式传入 `-access-token`，进程会按 `SUBCONVERTER_TOKEN` > 空值解析访问 token；空值表示当前 `/generate` 不启用鉴权。v2.0 Admin API 实现后，`/api/*` 默认要求 token：当 token 为空且未设置 `-allow-unauthenticated-admin` / `SUBCONVERTER_ALLOW_UNAUTHENTICATED_ADMIN=true` 时，返回 `401 admin_auth_required`。
...
本地开发若不想配置 token，可显式启动 `-allow-unauthenticated-admin`；不要在公网或不受信网络中开启该选项。
