...
| `REQ-02` | 支持多个 SS/AnyTLS 订阅源、Snell 来源与 VLESS 来源 |
| `REQ-03` | 支持自定义代理节点 |
| `REQ-04` | 支持链式节点与链式组生成 |
...
让生产部署通过 Docker Compose 启动单个 `subconverter` 服务：先运行 `pnpm web:embed` 构建 React SPA 并同步到 `internal/webui/dist`，再由根 Dockerfile 用 `webui` build tag 将已同步产物嵌入 Go 二进制，由同一个进程同源提供 SPA、`/api/*`、`/generate` 和 `/healthz`。M8 建立前端工程、嵌入式 Web 发布链路和生产入口，使 M9/M10 的前端开发可以独立迭代。

### 工作项

- 建立 `web/` 目录结构（React + Vite + TypeScript）
- 根 `pnpm web:embed` 构建 `web/dist/` 并同步到 `internal/webui/dist`
- 根 `Dockerfile` 使用已同步产物，并以 `-tags webui` 构建 Go 二进制
...
- `internal/webui/`：可选嵌入式 Web 静态资源入口；普通 Go 构建返回空 FS，`pnpm web:embed` 更新 `dist`
- 根 `Dockerfile`：使用已同步 Web embed 产物 + Go embed build + distroless runtime
...
- `pnpm web:embed` 可重复生成并同步 `internal/webui/dist`
...
- 前端代码更新后需要重新运行 `pnpm web:embed` 并重新构建生产镜像，使新的 `internal/webui/dist` 嵌入 Go 二进制
...
- 如果前端代码更新后忘记运行 `pnpm web:embed`，Docker 镜像会继续嵌入旧的 `internal/webui/dist`；CI 需校验该目录存在并在评审中检查产物同步
...
- 至少覆盖一个错误路径
- 不引入与当前里程碑无关的功能扩展
- 前端相关里程碑（M8-M10）额外要求 `pnpm web:test` 通过
