...
- `internal/admin/`：
  - `auth_handler.go`：登录、setup、logout 与 auth status handler（调用 `internal/auth`）
...
- API 请求使用同源 Cookie session；复制订阅链接前确认是否把订阅访问 token 写入 query
- `go test ./...` 全部通过
- 前端测试通过（`npm test`）
...
- B3 生成下载页面（调用 `GET/POST /api/generate/preview`、`/generate`、`GET /api/generate/link`，预览生成文本 + 下载按钮 + 复制订阅链接）
...
- 订阅链接复制功能正确：前端调用 `GET /api/generate/link`，显式确认后复制后端返回的 URL；服务端配置订阅 token 时链接可包含 `token` 和 `filename` 参数
