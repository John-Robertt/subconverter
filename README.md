...
- **多订阅源** — 支持 Base64、SIP002 `ss://`、Quantumult X 和 Surge 明文 SS 订阅源合并，跨订阅自动去重
...
`pnpm web:embed` 会在本地构建 `web/dist` 并同步到已提交的 `internal/webui/dist`。根 Dockerfile 只使用该嵌入产物和 Go 工具链构建镜像，不在 Docker 或 GitHub Actions 中运行 pnpm。构建产物默认命名为 `subconverter:local`。
...
sources:
  subscriptions:
    - url: "https://sub.example.com/api/v1/client/subscribe?token=xxx" # 订阅体可为 Base64、ss://、QuanX 或 Surge SS 明文列表
  snell:
...
```
发布期:
local pnpm web:embed --> internal/webui/dist --> go build -tags webui --> single binary
...
```
subconverter/
  Dockerfile             单镜像发布构建：使用已提交 Web embed 产物，再用 Go 输出 distroless runtime
  ...
  internal/
    ...
    webui/               可选嵌入 Web 静态资源；`internal/webui/dist` 由本地 `pnpm web:embed` 更新
