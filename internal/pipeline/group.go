...
// buildRegionGroups creates node groups by matching subscription proxies
// against each group's regex pattern, in groups declaration order.
// Only KindSubscription proxies participate in matching.
func buildRegionGroups(proxies []model.Proxy, groups *config.OrderedMap[config.Group]) ([]model.ProxyGroup, error) {
	subProxies := subscriptionProxies(proxies)
...
// from custom proxies that have relay_through definitions.
func buildChainedNodesAndGroups(
	...
	for _, cp := range customProxies {
		...
		upstreams, err := resolveUpstreams(subProxies, regionGroups, rt)
		if err != nil {
			return nil, nil, err
		}
...
// resolveUpstreams determines the upstream proxies for a relay_through definition.
func resolveUpstreams(
	subProxies []model.Proxy,
	regionGroups []model.ProxyGroup,
	...
	case "all":
		result := make([]model.Proxy, len(subProxies))
		copy(result, subProxies)
		return result, nil
...
// resolveMembers looks up proxy objects by name from the subscription proxy list.
func resolveMembers(subProxies []model.Proxy, names []string) []model.Proxy {
	index := make(map[string]model.Proxy, len(subProxies))
	for _, p := range subProxies {
		index[p.Name] = p
	}
...
// computeAllProxies collects names of original proxies (subscription + custom),
// excluding chained proxies. Called before chained nodes are generated.
...
// subscriptionProxies returns only proxies with Kind=KindSubscription.
func subscriptionProxies(proxies []model.Proxy) []model.Proxy {
	result := make([]model.Proxy, 0, len(proxies))
	for _, p := range proxies {
		if p.Kind == model.KindSubscription {
			result = append(result, p)
		}
	}
	return result
