...
- `401 token_required` / `401 token_invalid` 触发 token 输入，并能重试原请求。
- `401 admin_auth_required` 展示部署配置错误，不循环弹 token 输入框。
...
- token 默认只保存在内存。
- 选择会话记住时只写入 `sessionStorage`。
- `/api/*` 请求不把 token 放入 query。
- 复制含 token 的 `/generate` 链接前出现确认。
- 错误展示不泄漏原始含 token URL。
- 未配置 token 且未显式允许无鉴权 Admin 时，`/api/*` 无法被前端静默绕过。
