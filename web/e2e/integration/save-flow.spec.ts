// T-INT-SVF — Save workflow: validate → first-save confirm → PUT → reload.
...
test.describe("save flow", () => {
  test("T-INT-SVF-001 first save shows confirmation modal then succeeds", async ({ page }) => {
    ...
    const dialog = page.getByRole("dialog", { name: "确认首次写回 YAML" });
  ...
  test("T-INT-SVF-003 save succeeds but reload fails surfaces persistent warning", async ({ page }) => {
    const state = createMockState({ reloadMode: "fail" });
    await markFirstSaveConfirmed(page);
    await installMocks(page, state);
    ...
    await expect(page.getByText("配置已保存，reload 未完成")).toBeVisible();
    await expect(page.getByRole("button", { name: "重试 reload" })).toBeVisible();
  });
